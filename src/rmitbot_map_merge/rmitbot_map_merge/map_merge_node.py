...
class MapMergeNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("robot0_topic", "/robot_1/map")
        self.declare_parameter("robot1_topic", "/robot_2/map")
    ...
    def robot1_callback(self, msg):
        ...
        if not self.robot1_received:
            self.get_logger().info("Robot 2 map connected")
    ...
    def try_merge(self):
        ...
        try:
            if self.robot0_map is not None:
                trans0 = self.tf_buffer.lookup_transform('map', 'robot_1/map', rclpy.time.Time())
                self.tf0_x = trans0.transform.translation.x
                self.tf0_y = trans0.transform.translation.y
            
            if self.robot1_map is not None:
                trans1 = self.tf_buffer.lookup_transform('map', 'robot_2/map', rclpy.time.Time())
    ...
    def merge_maps(self):
        ...
        if not self.map_info_printed:
            ...
            if self.robot1_map is not None:
                self.get_logger().info("==============================")
                self.get_logger().info("Robot 2 Map")
                self.get_logger().info("==============================")
